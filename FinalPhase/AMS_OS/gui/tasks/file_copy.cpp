...
static void on_copy(GtkWidget *, gpointer) {
    ...
    if (!in.is_open() || !out.is_open()) {
        gtk_label_set_text(GTK_LABEL(status_lbl), "❌ Failed to open file(s)");
        ams_css(status_lbl, "error-text"); return;
    }
    out << in.rdbuf();
    gtk_label_set_text(GTK_LABEL(status_lbl), "✅ File copied successfully");
    ams_css(status_lbl, "success");
}

static void on_activate(GtkApplication *app, gpointer) {
    ams_apply_theme();
    GtkWidget *win = ams_window(app, "📋 Copy File", "edit-copy", 480, 300);
    ...
    GtkWidget *btn = gtk_button_new_with_label("📋 Copy"); ams_css(btn, "suggested-action");
    g_signal_connect(btn, "clicked", G_CALLBACK(on_copy), NULL);
...
int main(int argc, char *argv[]) {
    ...
    GtkApplication *app = gtk_application_new("com.ams.task.filecopy", G_APPLICATION_DEFAULT_FLAGS);
