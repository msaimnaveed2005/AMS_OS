...
static void on_create(GtkWidget *, gpointer) {
    ...
    if (strlen(path) == 0) {
        gtk_label_set_text(GTK_LABEL(status_lbl), "❌ Please enter a file path");
        return;
    }
...
static void on_activate(GtkApplication *app, gpointer) {
    ...
    GtkWidget *lbl = gtk_label_new("File Path");
    ams_css(lbl, "accent");
    gtk_widget_set_halign(lbl, GTK_ALIGN_START);
    gtk_box_pack_start(GTK_BOX(vbox), lbl, FALSE, FALSE, 0);

    path_entry = gtk_entry_new();
    gtk_entry_set_placeholder_text(GTK_ENTRY(path_entry), "data/virtual_disk/newfile.txt");
    gtk_box_pack_start(GTK_BOX(vbox), path_entry, FALSE, FALSE, 0);

    GtkWidget *clbl = gtk_label_new("Content (optional)");
    ams_css(clbl, "accent");
...
int main(int argc, char *argv[]) {
    ...
    GtkApplication *app = gtk_application_new("com.ams.task.createfile", G_APPLICATION_DEFAULT_FLAGS);
